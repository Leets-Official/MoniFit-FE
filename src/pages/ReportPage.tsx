...
function daysBetweenCeil(from: Date, to: Date) {
...
function calcSavedExceeded(total: number, budget: number) {
  const exceeded = Math.max(0, total - budget);
  const saved = Math.max(0, budget - total);
  return { saved, exceeded };
}

function getHalfWarnStorageKey(periodId: number) {
  return `monifit:budget:halfwarn:period:${periodId}`;
}

function HalfBudgetModal({ dailyWon, onClose }: { dailyWon: number; onClose: () => void }) {
  return (
    <div className="fixed inset-0 z-999 flex items-center justify-center">
      <button
        ...
        onClick={onClose}
      />
      <div className="relative w-80 rounded-xl bg-[#424242] px-4 pb-4 pt-7 shadow-[0px_10px_30px_rgba(0,0,0,0.35)]">
        <div className="flex flex-col gap-3">
          <h2 className="text-[16px] font-semibold text-white">예산의 50%를 초과했어요 ⚠</h2>
          <p className="text-[13px] leading-relaxed text-white/85">
            남은 기간 동안 하루{" "}
            <span className="font-semibold text-white">₩{Math.max(0, dailyWon).toLocaleString()}</span>{" "}
            으로 조절하면 목표 달성이 가능해요
          </p>
          ...
          <div className="pt-3">
            <button
              type="button"
              onClick={onClose}
              className="w-full rounded-[10px] bg-white px-4 py-3 text-[14px] font-semibold text-black"
            >
              닫기
            </button>
          </div>
...
export const ReportPage = ({ onClose }: ReportPageProps) => {
  ...
  const [halfDailyWon, setHalfDailyWon] = useState(0);
  ...
  const safeBudget = budgetValue ?? 0;
  const safeTotal = computedTotal;

  const derived = useMemo(() => {
    const baseTotal = totalValue > 0 ? totalValue : safeTotal;
    const { saved, exceeded } = calcSavedExceeded(baseTotal, safeBudget);

    const finalSaved = savedAmount > 0 ? savedAmount : saved;
    const finalExceeded = exceededAmount > 0 ? exceededAmount : exceeded;
  ...
  }, [totalValue, safeTotal, safeBudget, savedAmount, exceededAmount]);

  const resultText = useMemo(() => {
    if (derived.finalExceeded > 0) return `${derived.finalExceeded.toLocaleString()}원 초과 ⚠`;
    return `${derived.finalSaved.toLocaleString()}원 절약 🎉`;
  }, [derived.finalSaved, derived.finalExceeded]);
  ...
  useEffect(() => {
    ...
    const total = derived.baseTotal;

    if (budget <= 0) return;

    const ratio = total / budget;

    if (!(ratio > 0.5 && ratio < 1)) return;

    const key = getHalfWarnStorageKey(selectedPeriodId);
    const alreadyShown = localStorage.getItem(key) === "1";
    if (alreadyShown) return;

    const end = parseYMD(periodEndDate);
    const today = new Date();
    const today0 = new Date(today.getFullYear(), today.getMonth(), today.getDate(), 0, 0, 0, 0);

    if (!end) return;

    const daysLeft = daysBetweenCeil(today0, end);
    const remainingBudget = Math.max(0, budget - total);
    const daily = Math.floor(remainingBudget / daysLeft);

    setHalfDailyWon(daily);
    setIsHalfModalOpen(true);

    localStorage.setItem(key, "1");
  }, [hasPeriod, selectedPeriodId, loading, safeBudget, derived.baseTotal, periodEndDate]);

  return (
    <div className="relative z-50 flex h-screen w-full flex-col items-center bg-black pt-20">
      {isHalfModalOpen && <HalfBudgetModal dailyWon={halfDailyWon} onClose={() => setIsHalfModalOpen(false)} />}
